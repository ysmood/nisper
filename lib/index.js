...
module.exports = function (opts) {
    ...
    function genOnMessage (ws, env) {
        return function (msg) {
            ...
            return Promise.resolve(env).then(function (env) {
                if (raw === true)
                    return Nisp.exec(nisp, sandbox, env);
                else
    ...
    if (isClient) {
        ...
        clientCall = function (nisp, raw) {
            return call(wsClient, nisp, raw);
        };
        clientCallx = function () {
            var nisp = Nisp.encode.apply(Nisp, arguments);
        ...
        clientCall = function (nisp, filter, raw) {
            if (!filter) filter = function filter (cs) {
                return cs;
            };

            return Promise.all(filter(wsServer.clients).map(function (ws) {
                return call(ws, nisp, raw);
            }));
        };
        clientCallx = function () {
            var nisp = Nisp.encode.apply(Nisp, arguments);
            return clientCall(nisp, undefined, true);
        };
    }
